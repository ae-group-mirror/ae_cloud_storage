...
from ae.cloud_storage import DigiApi, GoodriveApi


# load configuration for integration tests (not on repository host CI)
load_dotenvs()
tst_digi_root_folder = os.environ.get('TEST_DIGI_ROOT_FOLDER_PATH', default="")
tst_digi_email = os.environ.get('TEST_DIGI_API_EMAIL', default="")
tst_digi_password = os.environ.get('TEST_DIGI_API_PASSWORD', default="")

tst_google_drive_root_id = os.environ.get('TEST_GOOGLE_DRIVE_ROOT_FOLDER_ID', default="")
...
# always run integration tests locally (@pytest.mark.integration would only enable them if RUN_INTEGRATION_TESTS is set)
@skip_gitlab_ci
class TestDigiApiIntegration:
    ...
    def test_deploy_file_update(self, digi_api_path_content):
        ...
        dir_list = api.list_dir(os.path.dirname(remote_path))
        assert dir_list
        assert dir_list[0] == tst_remote_file
...
@skip_gitlab_ci
class TestGoodriveApiIntegration:
